...
const MAX_ANIMATION_DURATION_S = 8;
...
function Physics() {
  ...
  const inputs: { label: string; value: number; setter: (value: number) => void; min: number; max: number }[] = [{ label: "Velocity (m/s)", value: velocity, setter: setVelocity, min: 1, max: 100 }, { label: "Angle (degrees)", value: angle, setter: setAngle, min: 1, max: 89 }, { label: "Gravity (m/s²)", value: gravity, setter: setGravity, min: 0.5, max: 30 }, { label: "Mass (kg)", value: mass, setter: setMass, min: 0.1, max: 100 }];
  return <Shell title="PHYSICS PLAYGROUND"><p className="text-fg-dim">Educational no-air-resistance model. Mass does not change projectile trajectory in a vacuum.</p><div className="mt-6 grid gap-4 sm:grid-cols-2">{inputs.map(({ label, value, setter, min, max }) => <label key={label} className="text-xs">{label}<input type="number" min={min} max={max} value={value} onChange={(e) => setter(Math.min(max, Math.max(min, Number(e.target.value) || min)))} className="ml-3 border border-border-dim bg-black-glass p-2" /></label>)}</div><div className="mt-5 flex flex-wrap gap-2">{[["Earth",9.81],["Moon",1.62],["Mars",3.71],["Jupiter",24.79]].map(([name,g]) => <button key={String(name)} className={button} onClick={() => setGravity(Number(g))}>{String(name)}</button>)}</div><div className="mt-8 border border-border-dim p-5">PROJECTILE / time {result.time.toFixed(2)} s · range {result.range.toFixed(1)} m · max height {result.height.toFixed(1)} m<br/><span className="text-xs text-fg-dim">Gravity drop: distance after one second is {(gravity / 2).toFixed(2)} m (from rest).</span></div></Shell>;
}
